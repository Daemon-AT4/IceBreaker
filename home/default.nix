...

{
  ...
  home.packages = with pkgs; [
    ...
    # NOTE: lsd, bat, fzf, zoxide, direnv are configured as
    # first-class home-manager modules below — do not also list
    # them here or shell integration may double-load.
    eza
    fd
    ripgrep
    tldr
  ...
  # Promote from raw package to first-class module so it picks up
  # Stylix theming and shell completions automatically.
  programs.lsd = {
    enable                = true;
    enableZshIntegration  = false;   # we define our own aliases in aliases.nix
  };
